...
from automation_shortcuts import get_desc

valid = {'dimension': [64, 128, 256, 512, 1024],
         'resolution_mm': [2, 3]}


def validate_params(dimension: int,
                    resolution_mm: int
                    ) -> NoReturn:
    """
    Asserts that the requested dimensions and resolution are available.
    """

    dimension_message = "\n".join((f"{dimension} is not a valid dimension.",
                                   f"Valid choices are {valid['dimension']}."))
    resolution_message = "\n".join((f"{resolution_mm} is not a valid resolution.",
                                    f"Valid choices are {valid['resolution_mm']}."))
    try:
        assert dimension in valid['dimension']
    except AssertionError:
        raise AssertionError(dimension_message)
    try:
        assert resolution_mm in valid['resolution_mm']
    except AssertionError:
...
def get_difumo(dimension: int,
    ...
    validate_params(dimension, resolution_mm)
